...
datarad<-read.table("Abies_nebrodi/genomic_analysis/RAD_diversity_poplevel.txt", header=T)
datarad$sp<-c("nebrodensis", "cephalonica","cephalonica","cephalonica", "alba", "alba", "alba")


datarad2<-read.table("Abies_nebrodi/genomic_analysis/RAD_diversity_poplevel2.txt", header=T)
datarad2$sp<-c("nebrodensis", "cephalonica","cephalonica","cephalonica", "alba", "alba", "alba")


layout(matrix(c(1,2),ncol=2))
boxplot(datarad$pi~datarad$sp, ylab="Nucleotide diversity", xlab="Alba species",
        ylim=c(0.19,0.23),col=brewer.pal(5,"Spectral"), main="With ind 25-14.3 (5 ind/pop)")

boxplot(datarad2$pi~datarad2$sp, ylab="Nucleotide diversity", xlab="Alba species", 
        ylim=c(0.19,0.23), col=brewer.pal(3,"Spectral"), main="Without ind 25-14.3 (4 ind/pop)")



##### All diversity statistics

datarad<-read.table("Abies_nebrodi/genomic_analysis/populations.sumstats_summary.tsv", header=T)
