                                                  ...
                                                  "STRUCTURE"), file = NULL, quiet = FALSE, sep = "/", hyb.label = "h") 
{
  ...
  freq1 <- tab(x1, freq = TRUE)
  freq1 <- split(freq1, y1@loc.fac)
  freq1 <- freq1[locNames(x1)]
  
  # r1<-unlist(lapply(freq1,sum))
  # rc1<-names(r1[r1==0])
  # freq1[rc1]<-lapply(freq1[rc1], append,1)
  
  
  y2 <- genind2genpop(x2, pop = factor(rep(1, n2)), quiet = T)
  freq2 <-tab(x2, freq = TRUE)
  freq2 <- split(freq2, y2@loc.fac)
  freq2 <- freq2[locNames(x2)]
  
  # r2<-unlist(lapply(freq2,sum))
  # rc2<-names(r2[r2==0])
  # freq2[rc2]<-lapply(freq2[rc2], append,1)
  
  
  
  kX1 <- lapply(freq2, function(v)  tryCatch(t(rmultinom(n, ploidy/2, 
                                               v)), error=function(err) matrix(NA, nrow=n, ncol=2)))
  ...
  kX2 <- lapply(freq1, function(v)  tryCatch(t(rmultinom(n, ploidy/2, 
                                                         v)), error=function(err) matrix(NA, nrow=n, ncol=2)))
