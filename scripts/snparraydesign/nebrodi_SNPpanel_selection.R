...
vcf_nebrodi <- read.vcfR(here("data","openarray_design",
"hybrids_filter","nebrodi_hybrids_snps_final.vcf"), verbose = FALSE )
...
# We select the top 10 alleles with the highest load from each of the 2 main PCs in the PCA
# We sort the alleles in descending order by load in absolute value
...
# SNP selection for hybridization ----
...
# Locus #542 has a high load on both axes 1 and 2, therefore we take one more from axis 1
# to make it 20
selectednebrodi_dapc_old<-nebrodi_genl[,c(dapc_sel1[-1],dapc_sel2)]

# Updated 12-1-2021, 7 additional hybridization loci are added after 7 loci from the first batch
# of arrays failed, therefore 15 are taken from axis 1 and 13 from axis 2 (27 = 28-1 common)
...
legend(-2.9,0.7, legend = c(expression(italic("A. alba")), 
...
write.table(newloci, file=here("data","openarray_design","hybrids_filter",
"nebrodi_hybrids_7snps_newdataset2021.txt"), col.names = F, row.names = F, quote = F)


# SNP selection for paternity test -----

#import vcf into geneind object

vcf_nebrodip <- read.vcfR(here("data","openarray_design","nebrodi_filter","clean_intergenicSNPs_nebrodi.vcf"), verbose = FALSE )
