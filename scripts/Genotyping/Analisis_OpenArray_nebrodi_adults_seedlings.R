##### Import A. nebrodensis adult and wild seedlings genotypes 
##### in Openarray format to genind
...
# Load, transform and rename replicates ----
...
colnames(new_GENOTYPE_CALLS) <- unique(GENOTYPE_CALLS2[,1]) #assign SNP ID
rownames(new_GENOTYPE_CALLS) <- unique( GENOTYPE_CALLS2[1:aa,2]) #assign Samples ID
...
# individuals that have completely failed
...
DF[DF$ID=="01_2019_2257", "a00161543_9085.G" ]<-0 #Detected a non-concordant replicate with genotype 1/1 instead of 2/0. Changed
# THERE ARE MORE DISCREPANCIES, REVIEW
...
# Filtering (it can be filtered after fixing the mothers)

#nebrodf<-missingno(nebrodf, type = "geno", cutoff = 0.50, quiet = FALSE, freq = FALSE)
# 10 more samples removed
...
# 0.05760301 +- 0.009546861 based on 598 samples
# the median is 0.01170569
